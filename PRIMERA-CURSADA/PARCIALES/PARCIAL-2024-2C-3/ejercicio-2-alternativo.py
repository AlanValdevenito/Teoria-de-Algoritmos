...
def largo_subsecuencia_balanceada_dinamico(cadena, n):
    ...
    for ventana in range(2, n):
        ...
        for i in range(n - ventana):
            ...
            elif (cadena[i] == '(') and (cadena[j] == ')'):
                mem[i][j] = 2 + (mem[i+1][j-1] if i+1 < j-1 else 0)

                print(f"Cadena: {cadena[i:j+1]} → Caso 2 → OPT({i}, {j}) = 2 + {(mem[i+1][j-1] if i+1 < j-1 else 0)}")

            elif (i + 1 < n) and (cadena[i] == '(') and (cadena[i+1] == ')'):
                mem[i][j] = 2 + (mem[i+2][j] if i+2 < j else 0)

                print(f"Cadena: {cadena[i:j+1]} → Caso 3 → OPT({i}, {j}) = 2 + {(mem[i+2][j] if i+2 < j else 0)}")

            elif (j - 1 >= 0) and (cadena[j-1] == '(') and (cadena[j] == ')'):
                mem[i][j] = 2 + (mem[i][j-2] if j-2 > i else 0)
                
                print(f"Cadena: {cadena[i:j+1]} → Caso 4 → OPT({i}, {j}) = 2 + {(mem[i][j-2] if j-2 > i else 0)}")
