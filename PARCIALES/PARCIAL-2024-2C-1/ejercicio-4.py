...
# 3 veces dicho elemento, si así lo quisiera (también menos cantidad). 
...
# Ecuacion de recurrencia: OPT(n,W) = max{usar/repetir el elemento, no usar el elemento} = max {OPT(n, W-Pi) + Gi, OPT(n-1, W)}
...
def mochila_solucion(elementos, n, W, mem):
    solucion = []
...
def mochila(elementos, W):
    ...
    solucion = mochila_solucion(elementos, n, W, mem)

    return solucion

# Complejidad

# Tenemos O(n x W) por calcular la matriz de memorizacion, donde n es el numero de elementos y W es la capacidad
...
# La matriz mem contiene los valores máximos que pueden lograrse para cada combinación de capacidad de la mochila (W) y cantidad de 
# elementos (n). La reconstrucción verifica si un elemento i fue incluido o no, observando cómo cambia el valor óptimo cuando se pasa 
# de n a n−1 en la matriz.
...
# El algoritmo termina cuando n = 0 o la capacidad restante de la mochila es W = 0.
